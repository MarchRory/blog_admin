...
<script>
  ...
  import articlePage from '/@/router/routes/modules/articles'
...
<script setup>
  import { ref, onMounted, toRaw, onBeforeUnmount, onActivated } from 'vue'
  ...
  import { BufferToMd } from '/@/utils/markdown/md'
  ...
  console.log(router.matched[0].components.default.value.loadArticleList)
  ...
  const init = () => {
    ...
    if (isNew.value === '1') {
      ...
      Object.keys(articleInfo.value).forEach((key) => {
        articleInfo.value[key] = ''
      })
      articleInfo.value.tagId = route.query.tagId
      return
    } else {
      ...
      getArticleDetail({ articleId: articleId.value })
        .then(async (res) => {
          if (typeof res === 'boolean') {
            ...
            const { data } = res
            contentEditor.value.setValue(await BufferToMd(data))
          }
        })
        .finally(() => {
          loading.value = false
  ...
  const handleSubmit = async () => {
    ...
    if (isNew.value === '1') {
      ...
      await addArticle({ ...articleInfo.value })
    } else {
      articleInfo.value.updateTime = new Date().getTime()
      await updateArticle({ ...articleInfo.value, articleId: articleId.value })
    }
    onSubmitSuccess()
    router.go(-1)
    router.matched[0].components.default.value.loadArticleList()
  }
  const onSubmitSuccess = () => {
    message.success(`文章${isNew === '1' ? '发布' : '更新'}成功`)
    onCancel()
  }
  ...
  const uploadCover = (cover) => {
    ...
    uploadCoverFile({
      ...
      .then((res) => {
        ...
        console.log('file: ', coverFileList.value) */
      })
      .catch((err) => {
