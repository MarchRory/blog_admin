...
<script setup lang="ts">
  ...
  import { ref, onMounted, defineAsyncComponent, watch, onBeforeUnmount, onActivated } from 'vue'
  ...
  const articleColumns: TableColumnsType = [
    ...
    { title: '发布时间', width: 60, dataIndex: 'publishTime', key: 'publishTime' },
    { title: '上次更新', width: 60, dataIndex: 'updateTime', key: 'updateTime' },
    { title: '浏览量', width: 40, dataIndex: 'views', key: 'views' },
  ...
  const edit = (record) => {
    router.push({ path: '/article/editArticle', query: { isNew: 0, ...record } })
  }
  const onConfirmDel = (articleId) => {
    delArticle({ articleId: articleId, tagId: selectedTagId.value as unknown as string }).then(
      () => {
        message.success('文章删除成功')
        if (articlePage.value.pageNum > 1 && articleList.value.length == 1) {
          articlePage.value.pageNum--
        }
        loadArticleList()
      },
    )
    ...
    >
      <div class="w-1/1 h-1/1">
        ...
        <a-table
          ...
          show-less-items
          :bordered="true"
          :scroll="{ x: true, y: tagTableHeight * 1.2 }"
        >
          <template #bodyCell="{ column, record }">
            <div v-if="column.key === 'operation'">
              ...
              <a-popconfirm
                title="确定删除这篇文章吗?"
                @confirm="onConfirmDel(record.articleId)"
                @cancel="onCancel"
              >
