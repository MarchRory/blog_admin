<script setup lang="ts">
  ...
  import { toRaw, ref, onMounted, defineAsyncComponent, watch, onBeforeUnmount } from 'vue'
  ...
  const handleTagRowClick = (record: tagListItem, index) => {
    return {
      onclick: () => {
        selectedTagRecord.value = JSON.parse(JSON.stringify(record))

        selectTagRowIndex.value = index
        console.log(selectedTagRecord.value)
      },
    }
  }
