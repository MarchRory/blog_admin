<script setup lang="ts">
  ...
  const tagValidatePass = async (_rule: Rule, value: string) => {
    if (value === '') {
      return Promise.reject('专栏至少要有名字')
    }
  ...
  const rules: Record<string, Rule[]> = {
    tagname: [{ required: true, validator: tagValidatePass, trigger: 'change' }],
  }
  const formState = ref({
    // 需要修改的表单内容
    tagname: '',
  ...
  <div class="p-4 h-1/1 flex md:flex enter-y flex-col justify-between">
    ...
    >
      <div class="p-6">
        ...
        >
          <a-form-item name="tagname" label="专栏名称" v-bind="formState.tagname">
            <a-input v-model="formState.tagname" style="width: 60%" placeholder="请输入专栏名" />
          </a-form-item>
          <a-form-item name="tagIcon" label="专栏logo" v-bind="formState.tagIcon">
            <Icon-picker
              :value="chosenIcon"
              width="60%"
              :page-size="30"
