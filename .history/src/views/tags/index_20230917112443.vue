<script setup lang="ts">
  ...
  const tagList: dataItem[] = ref([] as dataItem[])
  ...
  const loadList = () => {
    getTagList({ keywords: keywords.value, pageNum: pagination.value, pageSize: pagination.value }).then((res) => {
  ...
  <div class="p-4 h-1/1 flex md:flex enter-y flex-col justify-between">
    ...
    <div class="bar h-13/15 flex flex justify-start flex-col" style="background-color: white">
      ...
      <a-pagination
        ...
        :show-total="(total) => `共${pagination.total}项`"
      />
    </div>
  </div>
</template>
