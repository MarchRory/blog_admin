<script setup lang="ts">
  ...
  const tagInfoCopy = ref({} as formState)
  const handleSubmit = (info: formState) => {
    if (!info) {
      tagInfoCopy.value = JSON.parse(JSON.stringify(info))
    }
  }
      ...
      <div class="p-6">
        ...
        >
          <a-form-item
            ...
            type="text"
            label="专栏名称"
            v-bind="tagInfoCopy.tagname"
            :rules="[{ required: true, message: '专栏至少要有名字' }]"
          >
            <a-input
              type="text"
              v-model="tagInfoCopy.tagname"
