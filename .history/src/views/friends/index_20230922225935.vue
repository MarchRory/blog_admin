<script setup lang="ts">
  ...
  import { deleteFile, uploadAvatarFile, getBase64, FileItem } from '/@/api/sys/upload'
  import { deleteFileApiResult } from '/@/api/sys/model/uploadModel'
  ...
  const handleRemove = (file: File) => {
    ...
    deleteFile(name).then((res: deleteFileApiResult) => {
      message.success('头像移除成功')
    })
  }
  const uploadAvatar = (avatar) => {
    const file: File = avatar.file as File
    uploadAvatarFile({
      file,
      progressFn: onUploadProgress((ev) => {
        const percent = (ev.loaded / ev.total) * 100
        avatar.onProgress({ percent })
      }),
    })
      .then((res: any) => {
        formState.value.avatar = res
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          >
            ...
            <img class="object-cover h-80px" :src="record.avatar" alt="" />
          </div>
        </template>
      </a-table>
      <a-pagination
