<script setup lang="ts">
  ...
  const tableColumns: TableColumnsType = [
    ...
    { title: 'LOGO', width: 100, dataIndex: 'tagIcon', key: 'tagIcon' },
    { title: '专栏名', width: 100, dataIndex: 'tagname', key: 'tagname' },
    { title: '文章数量', width: 50, dataIndex: 'hasNums', key: 'hasNum' },
  ...
  const edit = (record) => {
    ...
    formState.value.tagname = copy.tagname
    formState.value.tagIcon = copy.tagIcon
    formState.value.tagId = copy.tagId
    console.log(formState.value)
    modalVisible.value = true
  }
  const onConfirmTag = (id) => {
    deleteTag(id).then((res) => {
      if (res === '专栏移除成功') {
        message.success(res)
        if (tagList.value.length === 1 && pagination.value.pageNum != 1) {
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'operation'">
            <div>
              ...
              <a-popconfirm
                title="确定删除该专栏吗?"
                @confirm="onConfirmTag(record.tagId)"
                @cancel="onCancel"
              >
